...
const String BASE95_CHARS = " !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~";
...
void changeL3Addres(int NewAddress){
  Serial.println("SparkFun VL53L5CX changing addres");

  Serial.println(F("Initializing sensor 8x8. This can take up to 10s. Please wait."));
  if (myImager.begin() == false)
  {
    Serial.println(F("Sensor 8x8 not found. Check wiring. Freezing..."));
    while (1) ;
  }

  Serial.print(F("Setting sensor 8x8 address to: 0x"));
  Serial.println(L3NewAddress, HEX);

  if (myImager.setAddress(L3NewAddress) == false)
  {
  ...
  Serial.print(F("New address of sensor 8x8 is: 0x"));
  ...
  bool response = myImager.setRangingFrequency(10);
  if (response == true)
  {
...
void changeL1Addres(){
  Serial.println("VL53L1CX changing addres");
...
void get_3x1_data(int arr[]) {
    for (uint8_t i = 0; i < sensorCount; i++)
  {
    arr[i] = sensors[i].read();
    //Serial.print(arr[i]);
...
void get_8x8_data(short int arr[]){
    ...
    if (myImager.getRangingData(&measurementData)) //Read distance data into array
    {
      ...
      for (int y = 0 ; y <= imageWidth * (imageWidth - 1) ; y += imageWidth)
      {
        for (int x = imageWidth - 1 ; x >= 0 ; x--)
        {
          ...
          arr[x + y] = measurementData.distance_mm[x + y];
        }
        //Serial.println();
      }
      String output = convertToBase95String(data_8x8,imageResolution);
...
String convertToBase95String(const short numbers[], size_t length) {
    String result = "";
    for (size_t n = 0; n < length; n++) {
        short num = numbers[n]; // Shift to make it always positive
        result += BASE95_CHARS[(num / 95) % 95];
        result += BASE95_CHARS[num % 95];
    }
    return result;
}

String packCharsUsing95(short num1, short num2) {
    Serial.println("\n\n------------------------------------\nEncode part");
    int pack = 0;
    short vec[] = {num1/10, num2/10};

    pack = (vec[0] << 9) | (vec[1]);

    Serial.println(pack,BIN);

    char high,mid,low;

    high = ((pack / 95) / 95) % 95 + 32;
    mid = (pack / 95) % 95 + 32;
    low = pack % 95 + 32;

    char packedStr[4] = {high, mid, low, '\0'};  

    return String(packedStr); 

}

String convert2To3By95(const short numbers[], size_t length) {
    String result = "";
    for(size_t n = 0; n < length; n+=2) {
      result += packCharsUsing95(numbers[n],numbers[n+1]);
    }
  return result;
}

bool getBit(short num, int position) {
    return (num & (1 << position)) != 0;
}

// Function to pack two chars into a short
void unpackChars(char high, char mid, char low, short &num1, short &num2) {
    int pack = (high << 16 ) | (mid << 8) | low;
    num2 = (pack >> 12) & 0xFFF;
    num1 = pack & 0xFFF;
}

// Function to unpack a short into two chars
String packChars(short num1, short num2) {
    int pack = 0;
    short vec[] = {num1/10, num2/10};

    pack = (num1/10 << 13) | (num2/10 << 1);

    //pack |= 0b100000000001100000000001;
    ...
    char packedStr[4] = {high+1, mid+1, low+1, '\0'};  // Null-terminated string
    if(String(packedStr).length()<3){
      Serial.println("tadá");
    }
    return String(packedStr); // Vytvoření String objektu z char pole
}

String short2string(const short numbers[], size_t length){
  String result = "";
  for(size_t n = 0; n < length; n+=2) {
    result += packChars(numbers[n],numbers[n+1]);
  }
  return result;
}
...
int tof_callback(int argc, char *argv[], char argv2[])
{
    ...
    String output = convert2To3By95(data_8x8,imageResolution);
    Serial.println(output);
    
    Serial.printf("tof %d %d %d %d %d %S\n",dist, tof_range1, tof_range3, tof_range2, strength, output.c_str());
    CommonSerial.printf("tof %d %d %d %d %d %S",dist, tof_range1, tof_range3, tof_range2, strength, output.c_str());
}
...
void setup()
{
  ...
  changeL3Addres(L3NewAddress); //changeing the I2C addres from the default 0x29 to 0x44
  changeL1Addres();             //changeing the I2C addreses of three L1 sensors
...
void tof_battery_read_task(void *arg)
{
    ...
    for (;;)
    {
      tof_range1 = sensors[0].read() / 10;
      tof_range2 = sensors[1].read() / 10;
      tof_range3 = sensors[2].read() / 10;

      get_8x8_data(data_8x8);
